...
`factory status` nags the open count until then.
After a `contract:` promotion, on the `claude-design` route (or for a legacy
config), if any `mcp__claude-design__*` tool is present and
`designsync_project` is configured, regenerate the linked
project's `factory-journeys.html` map (capabilities skill's
`references/designsync.md` `## Journeys`) — best-effort, never blocking.
Never file escapes on the factory's own behalf — this command is for what a
HUMAN found.
