...
| one rework figure | count of digit-bearing rework lines inside `## Cost decision` == 1, **and** every rework number on the packet's **rendered cost surfaces** agrees with it, **and** the only repetitions outside the block are the `- waiting on you:` derived echo and the `## Spend` receipt line — plus, once item 0015 ships, **two** permitted new lines on a redesigned item (enumeration completed 2026-08-03, judgement on bid-0120, after the walk found the second line unenumerated — the J-001 diff-(d) precedent applies: these enumerations bind by omission): (i) the labelled **`rework edges since last redesign`** companion, derived, whose cumulative echo agrees numerically with the primary figure (amended 2026-08-02, judgement on bid-0106 — an unlabelled or numerically-disagreeing companion still fails); and (ii) the **`redesigns: N of M`** counter, which is not a rework line at all (it counts engine-written `approach.rejected` edges, lifetime, never reset) and so is outside the one-rework-figure count by construction; `## Recent events` is excluded by name as an append-only verbatim audit dump — it records what was written, not what is aggregated, and must not be rewritten to agree; the count pattern matches both `rework edges: N` and `N rework edges` |
...
| gate off | with `"cost"` absent from `gates`: `over_threshold: true`, `fired: false`, no park, and packet/`status --json` byte-identical to the pre-change engine apart from the `rework edges` / `spend.rework_edges` renames — **scoped to items with zero `approach.rejected` edges** (carve-out added 2026-08-03, judgement on bid-0120: a redesigned item gains the two `## Spend` lines above with the gate off too, which S11's zero-approach-edge scoping already assumed but this oracle's text did not say). **Wall-clock caveat:** `status --json` is not byte-stable across seconds for an item with an open window (`active_seconds`, `elapsed_seconds`, `window.end`) — pre-existing in both engines, so this comparison requires excluding those fields or running a same-engine control |
