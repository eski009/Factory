...
def _round_two(mode, signals, conflicts, blocking_roles, prior_roles,
    ...
    if added_role:
        ...
        ordered = [SIGNAL_ROLE[entry["name"]] for entry in signals
                   if entry["name"] in SIGNAL_ROLE] + list(council.ROLES)
        next_role = next((role for role in ordered if role not in prior_roles), None)
        if added_role != next_role:
            raise ValueError("added role must be the next relevant omitted lens")
...
def _json_section(text, heading):
    if text.splitlines().count(f"## {heading}") != 1:
        raise ValueError(f"synthesis requires exactly one ## {heading}")
    section = _synthesis_section(text, heading).strip()
    match = re.fullmatch(r"```json\s*\n(.*?)\n```", section, re.DOTALL)
    if not match:
        raise ValueError(f"synthesis requires ## {heading} with one JSON block")
    def unique(pairs):
        result = {}
        for key, value in pairs:
            ...
            result[key] = value
        return result
    return json.loads(match.group(1), object_pairs_hook=unique)


def adjudication_errors(receipt, text):
    """Bind the final synthesis's explicit decision to its selection receipt.

    The engine checks recorded evidence, not the truth of arbitrary prose.
    Reviewers must finalize this block after the branch walk and synthesis.
    """
    try:
        data = _json_section(text, "Adjudication")
        expected_keys = {"verdict", "blocking_roles", "conflicts", "remaining_blockers"}
        if not isinstance(data, dict) or set(data) != expected_keys:
            raise ValueError("Adjudication requires verdict, blocking_roles, conflicts, remaining_blockers")
        for key in ("blocking_roles", "conflicts"):
            if data[key] != receipt["escalation"][key]:
                raise ValueError(f"Adjudication {key} must match finalized Round 1 escalation")
        if data["verdict"] != "approved" or data["remaining_blockers"] != []:
            raise ValueError("Adjudication must approve with no remaining blockers before verify")
    except (ValueError, TypeError) as exc:
        return [str(exc)]
    return []


def execution_errors(text, review_root):
    """Require a command/result record and digest-bound output, not a heading.

    This is reproducible recorded evidence, not host execution attestation.
    The reviewed command is never executed by the validator.
    """
    from .initrepo import evidence_file_digest
    try:
        records = _json_section(text, "Execution")
        if not isinstance(records, list) or not records:
            raise ValueError("Execution requires a non-empty record list")
        for record in records:
            if not isinstance(record, dict) or set(record) != {
                    "command", "exit_code", "observed_result", "output_file", "sha256"}:
                raise ValueError("Execution requires command, exit_code, observed_result, output_file, sha256")
            for key in ("command", "observed_result"):
                if not isinstance(record[key], str) or not record[key].strip():
                    raise ValueError(f"Execution {key} must be non-empty")
            ...
            if not isinstance(record["sha256"], str) or not re.fullmatch(r"[a-f0-9]{64}", record["sha256"]):
                raise ValueError("Execution sha256 must be a SHA-256 digest")
            if review_root is None:
                raise ValueError("Execution requires its review root for evidence validation")
            digest, error = evidence_file_digest(review_root, record["output_file"])
            if error or digest != record["sha256"]:
                raise ValueError("Execution output evidence missing, unsafe, or digest mismatch")
...
def receipt_errors(data, path, review_root=None, synthesis_text=None,
    ...
    for index, signal in enumerate(data["signals"]):
        ...
        elif any(not any(value == changed or value.startswith(changed + ":")
                         for changed in data["diff"]["changed_paths"])
                 for value in evidence):
            errors.append(f"{path}.signals[{index}].evidence: must cite a changed path")
    ...
    if round_number == 1:
        triggered = escalation["blocking_roles"] + [
            role for conflict in escalation["conflicts"] for role in conflict["roles"]]
        if any(role not in selected_roles for role in triggered):
            errors.append(f"{path}.escalation: triggers must name selected Round 1 roles")
        if escalation["prior_roles"] or escalation["added_role"]:
            errors.append(f"{path}.escalation: Round 1 has no prior or added role")
    if round_number == 2:
        if prior_receipt is None:
            ...
            if prior_errors:
                ...
                for key in ("blocking_roles", "conflicts"):
                    if escalation[key] != prior_receipt["escalation"][key]:
                        errors.append(f"{path}.escalation.{key}: must match Round 1 escalation")
    ...
    if synthesis_text is not None and degradation_required:
        ...
        if not _synthesis_section(synthesis_text, "Execution").strip():
            ...
            errors.extend(f"{path}: {error}" for error in
                          execution_errors(synthesis_text, review_root))
    return errors
