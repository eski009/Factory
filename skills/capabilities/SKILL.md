---
name: capabilities
description: Use when a factory skill needs to fan out work, render design options, or schedule runs - defines probe-and-upgrade for optional tools
---

Factory skills share one host-neutral contract across Codex and Claude Code,
then upgrade opportunistically when a tool happens to be available. Resolve
plugin-root commands and host-native tools through `references/host-adapter.md`.
This skill is the one place that general degradation logic lives; every stage
skill cites it instead of re-deriving it.

| Capability | Probe | With it | Without it |
|---|---|---|---|
| Workflow tool | tool present in tool list | Fan out council rounds and independent plan tasks via Workflow → see references/workflow-fanout.md | Parallel Task subagent dispatches in one message |
| Artifact tool | tool present in tool list | Publish the options page as an Artifact — the standard way design options are shown to a human in an interactive session, so they open on phone or desktop from one link → see references/artifact-hosting.md | Write HTML to `items/<id>/design/options.html` and tell the user to open it |
| DesignSync | `design.provider: claude-design` (or a legacy config) and any `mcp__claude-design__*` tool present in the tool list, or the built-in `DesignSync` tool | Pull/push the linked Claude Design project → see references/designsync.md | Legacy configs use `docs/factory/brain/design-system.md`; an explicit `claude-design` selection pauses with the missing setup instead of silently switching provider |
| Scheduled agents | tool present in tool list | `loop` mode runs on a schedule → see references/scheduling.md | User runs `/factory:run loop` manually |
| Browser read-back | a browser-automation tool that can read page DOM/console is present in the tool list | Open the options page in the controlled browser; after the human clicks "Record choice", read the finalized `#factory-choice` state (or the `FACTORY_CHOICE` console line) and run `factory choice <id> <opt> --notes "…"` on their behalf → see references/browser-read.md | Human copies the page's composed `factory choice` command (or the packet's verbatim CLI line) and runs it |
| Browser drive | a browser-automation tool family that can navigate, click, type, screenshot, and read console/network is present (Playwright MCP, chrome-devtools MCP, Claude-in-Chrome) | factory-assure's journey-reviewer drives browser-borne journeys and captures screenshot/console/network evidence → see references/browser-drive.md | Browser-borne journeys are a blocker: the item parks waiting-human with a packet naming `factory waive` — never a silent pass; cli/api journeys proceed unaffected |
| App visual capture | a screenshot-capable driver for the target app is present in the tool list — `mcp__maestro__*`/`mcp__maestro-local__*` for an iOS sim, a browser-automation tool or Playwright for web | Verify a visual / `mode: human-confirmed` bug with the blind observer protocol: a fresh subagent drives the app and reports what it factually sees, blind to the diagnosis/diff/expected, and verify judges that report → see references/visual-verify.md | Verify cannot self-attest a visual criterion — it does not log `verify.green`; the item pauses `waiting-human` for the human to confirm the repro steps |
| Headless worker | `workers.enabled` true in `.factory/config.json` and the configured backend CLI (`claude`/`codex`) resolvable on `PATH` with its key env var set (or, for codex, `workers.codex.auth: "chatgpt"` plus a fresh `codex` login — `factory doctor` reports both) | Dispatch an item's implementation out-of-process via `factory work <id>`; with several independent items at `implement`, run the bounded parallel pool (the `factory-workers` skill) → see references/headless-workers.md | Today's in-process `superpowers:subagent-driven-development` path, unchanged |

Probe by attempting nothing: check the tool list. Never let a missing optional
tool fail a stage unless the user explicitly selected that tool as the stage's
provider — the degraded path is otherwise the contract, and upgrades are
opportunistic. In a fresh subagent context, optional MCP tools may be deferred
rather than listed — attempt to load them via tool search before concluding
absence; a tool that cannot be loaded is absent, take the documented branch.

**Process patterns** (not capability-gated — these apply regardless of which
tools or model are available): see references/orchestration-patterns.md for the
seven patterns that make outcomes reproducible,
`references/disk-first-reconciliation.md` for every parent-child dispatch whose
reply may be lost, and references/model-tiering.md for the abstract model tiers.
Before choosing the lead or dispatching work, read `references/model-routing.md`
for Claude-hosted model roles or `references/model-routing-codex.md` for Codex
model and effort recommendations.

Apply this row by row: check the tool list once per capability you need, take
the matching branch, and move on. Don't ask the user whether a tool is
available, and don't retry a probe that already came back negative. Follow the
"Without it" column, including its explicit-provider pause where stated.
