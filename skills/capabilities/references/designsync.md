...
DesignSync is the factory's name for mirroring design tokens and built UI between the repo and a linked Claude Design project, via the `mcp__claude-design__*` MCP tool family. It is only useful when that tool family and an interactive claude.ai login are available. For legacy repositories the degraded path — repo-local `docs/factory/brain/design-system.md` tokens — remains sufficient. When the user explicitly selects `design.provider: claude-design`, missing MCP or project setup pauses design rather than silently changing providers. `docs/factory/brain/design-system.md` and `items/<id>/design/` stay canonical and cold-resumable; the linked project is a preferred interactive mirror, never a second source of truth.
...
`factory doctor` surfaces `design_provider` and `designsync_project` as plain readouts. Doctor itself never gates. `factory-design` decides the runtime branch: legacy configs fall through to `design-system.md`; explicit `claude-design` configs pause visibly when the selected integration is unavailable.

## Interactive-only

The `mcp__claude-design__*` family depends on an interactive claude.ai login, so it is never attempted in headless, scheduled, or autopilot runs (see `references/scheduling.md`). Legacy configs skip the pull/push and rely on `design-system.md`. An explicit `claude-design` selection instead parks the design item with a packet naming the unavailable MCP or project link; it never records a false provider success or silently uses Codex.
