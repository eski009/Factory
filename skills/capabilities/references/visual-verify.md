...
The failure this prevents: a `mode: human-confirmed` (visual) bug reaches `verify` in an
unattended context. The verify agent has no runnable command, but it *can* read
...
in a subagent or otherwise deferred-tool context, attempt to load a candidate driver via
ToolSearch before concluding absence.
