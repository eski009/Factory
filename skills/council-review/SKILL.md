...
This skill runs in a forked context (`context: fork`): nothing from the invoking session is visible here. The stage `mode` is `triage`, `review`, or `research`. For review, the distinct `selection_mode` is `adaptive` or `full`. The skill argument also names the item id (or, for research mode, the research root); every other input is read from disk per the mode list below. Your final message is the report the invoking stage acts on: the synthesis file path, the headline verdict, and the blocking-finding count — a few lines, never the report bodies.
