...
- **Spend.** The base walk logs its own spend event: `factory log ITEM spend --data '{"provenance":"measured","scope":"leaf","stage":"assure","source":"factory-assure-base","dispatches":<n>,"tokens":{"total":<n>}}'`, or `"provenance":"proxy","scope":"leaf"` with no `tokens` key when the harness reports none.
...
Log one spend event per reviewer dispatch batch, per the dispatch convention: `factory log ITEM spend --data '{"provenance":"measured","scope":"leaf","stage":"assure","source":"factory-assure","dispatches":<n>,"tokens":{"total":<n>}}'` with harness-reported counts, or `"provenance":"proxy","scope":"leaf"` and no `tokens` key when the harness reports none. Never estimate.
