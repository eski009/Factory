...
The run-summary packet also carries a `## Spend` section: one line per item touched this run, in the form `- <id>: [proxy] active <dur>, <n> dispatches; [measured] tokens <observed keys, e.g. total <n>>` (render only the token keys that were actually logged nonzero for that item — never fabricate `input 0`/`output 0` for a split nobody reported — or `; [measured] tokens: none logged` when that item has no measured spend events) — read each item's figures from `factory cost <id> --json` — and **no run-total line**: no total, sum, mean, or median across items appears anywhere in the packet, because the spend event classes measure different quantities and a cross-item figure carries no provenance class of its own. Never collapse the run into a single blended number, and never render the orchestrator's own unmeasured main-loop burn as zero or a dollar figure.
