...
0. **Resume check.** Before pulling any new work, look for answered pauses: `factory status --json`, filter to `stage == "waiting-human"`. For each, check whether the artifact it's waiting on now exists. Today that means: items paused from `design` (`meta["paused-from"] == "design"`) whose `.factory/items/<id>/design/choice.md` is now present and non-empty; and items paused from `assure` whose `.factory/items/<id>/assurance/waiver.md` or `.factory/items/<id>/assurance/human-confirmation.md` is now present and non-empty (the human ran `factory waive` or `factory confirm`) — or, when its `paused-reason` names an undeclared journey impact — whose `journeys` field is now set (both read from the same `factory status --json`). And: items paused with a `paused-reason` beginning `cost breaker:` whose `.factory/items/<id>/cost/answer.md` is now present and non-empty — those resume **only when the artifact records `- answer: continue`**. `narrow` and `defer` leave the item parked and the human drives; routing on the recorded option belongs to this skill, never the engine — the same seam as `- option: none` below. And: items paused with a `paused-reason` beginning `approach cap:` whose `.factory/items/<id>/approaches/answer.md` is now present and non-empty — those resume **only when the artifact records `- answer: continue`**: `factory advance ITEM <paused-from>`, then immediately re-issue the redesign request — `factory advance ITEM spec --reason "approach.rejected: <the one-line reason>"` — which the recorded answer admits exactly once. `narrow` and `defer` leave the item parked and the human drives; routing on the recorded option belongs to this skill, never the engine. If so, `factory advance ITEM <paused-from>` (the stage read from that item's own status JSON) and treat that item as the current one for this pass instead of calling `factory next`. On a successful resume advance, delete that item's answered packets — `docs/factory/packets/<id>.md`, `docs/factory/packets/<id>.html`, `docs/factory/packets/<id>-design.md`, and `docs/factory/packets/<id>-assure.md`, whichever exist — the log is the durable record, and a stale packet left behind misleads the hook's "awaiting review" listing. Do this in every mode, not just loop. In item mode, apply the resume-advance only when the resumed item is the currently tracked item; resumed answers on other items are left for a later pass (loop mode picks them up naturally). Resume always returns the item to its own `paused-from` stage regardless of which option `choice.md` records — `- option: none` resumes to `design` exactly like a pick does. Dispatch never advances a design item toward `plan` based on `choice.md` content; pick-vs-none routing belongs exclusively to factory-design's entry check.
...
- Any item that enters `waiting-human` — whether via a stage skill's own gate or the mapped-skill-unavailable case above — always gets `factory packet ITEM` before you continue or stop. The packet also writes the visual `docs/factory/packets/<id>.html`; surface that HTML link, or a hosted Artifact URL from the paused reason, as the primary "view the options" link ahead of the Markdown packet.
- **Cost breaker.** After any `factory advance ITEM implement` whose stdout carries a `cost breaker:` line, park the item — `factory advance ITEM waiting-human --reason "cost breaker: <n> rework edges (threshold <t>)"`, copying `<n>` and `<t>` from that line — then `factory packet ITEM` (the rule above). The advance itself exits `0` and returns the requested stage: the breaker is advisory, so it is your job to park, not the engine's. In **loop** mode, continue with the next actionable item in the backlog; in **item** or **step** mode, stop the run here. Never re-run the advance to "get past" the breaker: the next entry into `implement` is refused by the engine until `factory cost-answer` records an answer.
- **Approach cap.** After any `factory advance ITEM spec` refused with stderr carrying `approach cap:`, park the item — `factory advance ITEM waiting-human --reason "approach cap: <n> redesign(s) used (cap <m>)"`, copying `<n>` and `<m>` from the refusal — then `factory packet ITEM` (the rule above). In **loop** mode, continue with the next actionable item in the backlog; in **item** or **step** mode, stop the run here. Never re-run the advance to "get past" the cap: the engine refuses every firing-set → spec request until `factory approach-answer` records an answer whose watermark covers the current count, and each recorded answer admits exactly one more redesign.
- **Verify failures route to their remedy.** When factory-verify reports a failure whose fix lives in the implementation (it names the failing criteria and a remedy), route the rework — `factory advance ITEM implement --reason "verify rework: <one line>"` — never park it `waiting-human` for a failure verify has already diagnosed. When the report says the design itself cannot converge, the rejecting stage writes `.factory/items/ITEM/approaches/forbidden.md` first, then requests `factory advance ITEM spec --reason "approach.rejected: <one line>"`.
...
At every subagent fan-out point for an item, when each dispatch (or batch) completes, the orchestrating session logs one spend event: `factory log ITEM spend --data '{"provenance":"measured","scope":"leaf","stage":"<stage>","source":"<skill>","dispatches":<n>,"tokens":{"total":<n>}}'` (include `"input"`/`"output"` instead or additionally when the harness reports them) using the token counts the harness reports for those subagents. If the harness surfaces no token usage, log the same event with `"provenance":"proxy","scope":"leaf"` and **no** `tokens` key. Never estimate or invent token numbers; the orchestrator's own main-loop burn is never logged as measured. Stage skills apply this same convention at their own fan-out points; the engine neither requires nor verifies these events at gates. A fresh stage-skill invocation is itself a subagent dispatch under this convention: when it returns, log one spend event for it — `{"provenance":"measured","scope":"fork","stage":"<stage>","source":"factory-dispatch","dispatches":1,"tokens":{...}}` with the token count the harness reports for the fresh context, or `"provenance":"proxy","scope":"fork"` with no `tokens` key when it reports none. Scope is assigned at origin and never inferred: stage-owned inner dispatches or sibling aggregates are `leaf`; the dispatcher-owned containing stage invocation is `fork` because its usage can contain those child events. Provenance remains independent of scope.

## Context hygiene

Stage skills run in fresh host-native contexts and dispatch their own subagents
for heavy work (reading specs, writing code, reviewing diffs); each stage's
working context is discarded when it returns, and only its compact outcome
report enters this session. The dispatcher itself never reads item artifacts
beyond metas and those reports — keep this session's context to routing
decisions, not item content.
