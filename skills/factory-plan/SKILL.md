...
- **Complete code, not descriptions:** every task carries the exact code, exact tests, and exact commands (with expected output) it needs — a task is done when the implementer's job is transcription, never invention. This is what lets a cheap-tier implementer produce correct code every time: see `skills/capabilities/references/orchestration-patterns.md` pattern 1. A task that says "add appropriate handling for X" instead of showing the handling is not plan-complete.
