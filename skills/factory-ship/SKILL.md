...
- **Entry gate:** for journey-affecting items that run the assure stage, `assure.passed` (or a recorded human waiver) after the latest implementation round — plus `assure.confirmed` (equally round-scoped) when the repo's config gates include `assure` and no waiver is recorded, a waiver being authoritative. For `journeys: none` items and items whose immutable bug-intake event gives status the derived mode `assurance: verify`, fresh round-scoped `verify.green` substitutes for the separate assure stage. The engine's `_gate_ship` is the authority.
