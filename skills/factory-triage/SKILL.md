...
2. Run the `council-review` skill in **triage mode** (pass `mode: triage` and the item id as the skill argument): should this be built, at what priority, with what scope cuts. The council also proposes new items from brain surfaces (open questions, decisions, scan findings) during this pass — file each proposal with `factory add "title" --kind ...`. Log the fan-out's spend before moving on: `factory log ITEM spend --data '{"provenance":"measured","stage":"triage","source":"council-review","dispatches":<n>,"tokens":{"total":<n>}}'` when the harness reports token counts, or `"provenance":"proxy"` with `"dispatches":<n>` and no `tokens` key when it does not. Never estimate. An unlogged triage fan-out is why `factory cost` under-reports by a whole council, and the packet now renders a loud `[unmeasured] stage triage` line when this event is missing.
...
6. **Set the materiality tier.** Decide the item's tier from its scope — `epic` (a material feature or UI-heavy initiative that warrants the focus group and full review), `feature` (a normal change: full review, standard research), or `bug` (a defect fix: light correctness review, no market research). A defect filed through `/factory:bug` carries `tier: bug` already — but as of 2026-08-03 no item in this repo has taken that door (0 of 32 carry `bug: true`), so assume nothing and set the tier here yourself. Set it with `factory tier ITEM <epic|feature|bug>`. The council's scope read informs this; when a human is present and the call is close, ask. Tier is a materiality axis, **orthogonal to `kind`** — a `backend` epic and a `ui` feature are both normal. Default when genuinely unsure: `feature`.
