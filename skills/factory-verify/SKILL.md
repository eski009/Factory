...
- **Exit — all pass:** `factory log ITEM verify.green --data '{"tests": "<counts>", "criteria": "<n>/<n>"}'`, then read the item's `journeys` and derived `assurance` value from `factory status --json`. Attempt `factory advance ITEM ship` when status carries `journeys: none` **or** `assurance: verify`; otherwise attempt `factory advance ITEM assure`. The engine gate is the authority — if it refuses, report the message verbatim, never guess a different stage. The assure entry gate — and, on either verify-substituted path, the ship gate — checks only for the fresh, round-scoped `verify.green` event; that is exactly why it must never be logged on partial evidence.
- **Exit — any failure:** no `verify.green`, no forward advance. When the failure's remedy lives in the implementation (a failing test, a wrong branch, prose contradicting behavior), report it to the dispatcher naming the remedy — the dispatcher routes the rework via `factory advance ITEM implement`, never a `waiting-human` park for a failure this stage has already diagnosed. When the evidence says the *design itself* cannot converge (a good implementation of the wrong approach), append a dated entry to `.factory/items/ITEM/approaches/forbidden.md` — a heading with timestamp and this stage, one paragraph naming the approach tried, and repo-relative paths to the evidence — then request the redesign: `factory advance ITEM spec --reason "approach.rejected: <one line>"`. Anything else is a plain stage failure; the dispatcher applies its own two-strikes-then-blocked rule.
...
5. Only if the suite is fully green **and** every criterion's line shows a pass: log `verify.green` with the real test counts and `criteria` as `"<passed>/<total>"` (adding the `visual` evidence key from the capabilities skill's `references/visual-verify.md` when any criterion was visual), then advance per the Contract.
