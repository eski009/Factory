...
def valid_receipt(item="0001-x", repo=None):
    ...
    if repo is not None:
        ...
        root = Path(repo) / ".factory/items" / item / "reviews"
        root.mkdir(parents=True, exist_ok=True)
        (root / "synthesis.md").write_text(synthesis_text(data), encoding="utf-8")
    return data


def synthesis_text(receipt):
    return "# Review\n\n## Adjudication\n\n```json\n" + json.dumps({
        "verdict": "approved", "remaining_blockers": [],
        "blocking_roles": receipt["escalation"]["blocking_roles"],
        "conflicts": receipt["escalation"]["conflicts"],
    }) + "\n```\n"


def execution_text(root):
    output = b"OK\n"
    (root / "probe.txt").write_bytes(output)
    return "\n## Execution\n\n```json\n" + json.dumps([{
        "command": "python3 -m unittest tests.test_review_selection -q",
        "exit_code": 0, "observed_result": "OK",
        "output_file": "probe.txt", "sha256": hashlib.sha256(output).hexdigest(),
    }]) + "\n```\n"
...
class ReceiptTest(unittest.TestCase):
    def test_static_only_execution_and_tampered_output_are_refused(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            for prose in ("Static inspection only; no command or probe ran.",
                          "Command: none\nObserved result: not run"):
                self.assertTrue(selection.execution_errors(
                    "## Execution\n" + prose, root))
            text = execution_text(root)
            self.assertEqual(selection.execution_errors(text, root), [])
            (root / "probe.txt").write_text("different output")
            self.assertTrue(selection.execution_errors(text, root))
            (root / "probe.txt").unlink()
            (root / "probe.txt").symlink_to(root.parent / "outside")
            self.assertTrue(selection.execution_errors(text, root))

    def test_adjudication_must_match_finalized_escalation_and_be_clean(self):
        receipt = valid_receipt()
        clean = synthesis_text(receipt)
        self.assertEqual(selection.adjudication_errors(receipt, clean), [])
        receipt["escalation"]["blocking_roles"] = ["architecture"]
        found = synthesis_text(receipt)
        self.assertTrue(selection.adjudication_errors(valid_receipt(), found))
        self.assertTrue(selection.adjudication_errors(receipt, clean))
        self.assertTrue(selection.adjudication_errors(
            receipt, found.replace('"approved"', '"rejected"')))
        self.assertTrue(selection.adjudication_errors(receipt, "# Review\n"))
    ...
    def test_degraded_receipt_requires_non_empty_execution_section(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for outcome in data["outcomes"]:
                if outcome["report"]:
                    report = root / outcome["report"]
                    report.parent.mkdir(parents=True, exist_ok=True)
                    report.write_text("returned\n")
            synthesis = degradation + execution_text(root)
            self.assertEqual(selection.receipt_errors(
                data, "receipt", review_root=root, synthesis_text=synthesis), [])
    ...
    def test_refuses_round_two_cross_subject_receipts(self):
        ...
        round_one["escalation"]["blocking_roles"] = ["architecture"]
        self.assertEqual(selection.receipt_errors(
